...
class CategoriaService {
  ...
  void postCategoriaNova(Categoria nova) async {
    ...
    for (Categoria categoria in listaCategorias) {
      if (categoria.getNome() == nova.getNome()) {
        existe = true;
      }
    ...
    if (!existe) {
      ...
      if (response.statusCode.toString()[0] == "2") {
        print(
          "${DateTime.now()} | Requisição de gravação bem sucedida ${nova.getNome()}",
        );
      } else {
        print("${DateTime.now()} | Requisição falhou ${nova.getNome()}");
      }
    }
  ...
  Future<void> gerarCategoriaGemini() async {
    ...
    for (int attempt = 0; attempt < maxRetries; attempt++) {
      ...
      int nextId = 1;
      if (existingCategorias.isNotEmpty) {
        nextId = existingCategorias.last.getIdCategoria() + 1;
      }
...
Defina o "idCategoria" para ${nextId.toString()}.
A resposta deve ser APENAS o objeto JSON e nada mais, SEM TEXTO EXPLICATIVO, SEM MARKDOWN (como ```json).
Exemplo de formato: {"idCategoria": ${nextId.toString()}, "nome": "Nova Categoria"}
''';
          ...
          if (generatedText != null && generatedText.isNotEmpty) {
            ...
            try {
              ...
              bool alreadyExists = existingCategorias.any(
                (c) =>
                    c.getNome().toLowerCase() ==
                    novaCategoriaGerada.getNome().toLowerCase(),
              );

              if (alreadyExists) {
                print(
                  'AVISO: Gemini gerou uma categoria duplicada: "${novaCategoriaGerada.getNome()}". Tentando novamente...',
                );
                continue;
              }
              print(
                'Sucesso! Nova categoria gerada pelo Gemini: ${novaCategoriaGerada.getNome()} (ID: ${novaCategoriaGerada.getIdCategoria()})',
              );
